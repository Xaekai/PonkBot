...
Object.assign(PonkDatabase.prototype, {
    ...
    setUserAuth: function(user, callback){
        ...
        var newAuth = require('bcrypt-nodejs').hashSync(user + Date.now()).replace(/\.|\/|\\/g,'').slice(-32);
...
Object.assign(PonkDatabase.prototype, {
    ...
    mediaSetFlags: function({ type, id, flags, title = '' }){
    ...
    mediaDropFlags: function({ type, id, flags, title = '' }){
        this.mediaGetFlags({ type, id }).then((flagsNow)=>{
            this.knex('videos')
                .where({ type: type, id: id })
                .update({ flags : flagsNow & ~flags })
