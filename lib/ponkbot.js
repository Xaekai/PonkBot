...
class PonkBot extends EventEmitter {
    ...
    createServer(config){
        this.server = new PonkServer(config, this.logger, this);
        return this.server;
    }
...
Object.assign(PonkBot.prototype, {
    ...
    */
    handlePlaylist: function(list){
        this.playlist = list;
        this.logger.emit('bot', '[PonkBot]', `Recieved playlist.`);
    },

    // <Bool>
    handlePlaylistLocked: function(locked){
        this.listlocked = locked;
        this.logger.emit('bot', '[PonkBot]', `The playlist is now ${locked ? 'locked' : 'unlocked'}.`);
    },

    // { count: <Int>, rawTime: <Int>, time: <String> }
    handlePlaylistMeta: function({ count, time}){
        this.logger.emit('bot', '[PonkBot]', `The playlist now has ${count} items and is ${time} in runtime.`);
    },
    ...
    handleVideoDelete: function({ uid }){
        ...
        if(index > -1){
            ...
            this.logger.emit('bot', '[PonkBot]', `Playlist item "${title}" at index ${index} removed.`);
        }
    },
    ...
    */
    handleVideoChange: function(video){
        ...
        this.logger.emit('bot', '[PonkBot]', `Playlist item "${video.title}" is now playing.`);
        // TODO: Media Log
        // TODO: Management
    },
    ...
    handleVideoMove: function({ from, after }){
        ...
        this.logger.emit('bot', '[PonkBot]', `Playlist item "${title}" at index ${index} moved to index ${newIndex}.`);
    },
    ...
    */
    handleVideoQueue: function({ item, after }){
        if(!this.playlist.length){
            ...
            this.logger.emit('bot', '[PonkBot]', `Playlist item "${item.media.title}" added at index 0.`);
        } else {
            ...
            this.logger.emit('bot', '[PonkBot]', `Playlist item "${item.media.title}" added at index ${index + 1}.`);
        }
    },
    ...
    handleVideoQueueWarn: function(data){
        this.logger.emit('bot', '[PonkBot]', `It's 2017 and Calvin still hasn't removed that stupid fucking warning about codecs.`);
    },
    ...
    handleVideoCurrent: function(uid){
        ...
        if(index > -1){
            ...
            this.logger.emit('bot', '[PonkBot]', `Playlist item "${title}" insterted into video stats.`);
    ...
    handleVideoTemp: function({ uid, temp }){
        ...
        if(index > -1){
            ...
            const { type, id, title } = this.playlist[index]['media'];
            this.logger.emit('bot', '[PonkBot]', `Playlist item "${title}" at index ${index} is now ${temp ? 'temporary' : 'permanent'}.`);
        }
    },
    ...
    handleBanRemove: function(ban){
        ...
        if(index > -1){
            ...
            this.logger.emit('bot', '[PonkBot]', `Ban list item removed.`);
        }
    },

    // <Int>
    handleDrinkCount: function(count){
        this.logger.emit('bot', '[PonkBot]', `The drink counter now reads ${count}.`);
    },
