    ...
    */
    ...
    checkPermission({ user, rank = 1<<16, hybrid = [] }){
        ...
        return new Promise((resolve, reject) => {
            ...
            return reject(`You don't have permission to use this command.`);
        });
    }
    ...
    mediaSend({ type, id, temp = true, pos = 'end' }) {
        const media = {
            'type'     : type,
            'id'       : id,
            'pos'      : pos,
    ...
    mediaValidate(playlistItem){
        ...
        const handleFlags = (flags) => {
            console.log('checking flags, FLAGS.MEDIA.BLACK, rank', flags, FLAGS.MEDIA.BLACK, rank)
    ...
    userCheckFlags(user, flag){
        return new Promise((resolve)=>{
            this.db.userGetFlags(user).then((flags)=>{
                resolve(flag & flags);
            });
        });
    }
...
PonkBot.prototype.mediaAddFlags = function({ type, id, flags, title = '' }){
    this.db.mediaSetFlags({ type, id, flags, title });
}

PonkBot.prototype.mediaGreylist = function({ type, id, title = '' }){
    this.mediaAddFlags({ type, id, flags: FLAGS.MEDIA.GREY });
}

PonkBot.prototype.mediaWhitelist = function({ type, id, title = '' }){
    this.mediaAddFlags({ type, id, flags: FLAGS.MEDIA.WHITE });
}

PonkBot.prototype.mediaBlacklist = function({ type, id, title = '', deletThis = true }){
...
PonkBot.prototype.mediaFlagByUID = function({ flag, uid }){
