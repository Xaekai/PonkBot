...
class PonkServer extends EventEmitter {
    ...
    setupRoutes(app){
        ...
        */
        ...
        this.availableLogs = {
            ponklog  : { file: 'ponkbot.log', label: 'Bot Log' },
            errorlog : { file: 'error.log',   label: 'Error Log' },
            medialog : { file: 'media.log',   label: 'Media History' },
        }

        app.get('/logs/:logid', (req, res) => {
            const logid = req.params.logid;

            if(!(logid in this.availableLogs)){
                return res.send('Invalid log id').sendStatus(500);
            }

            this.readLog(path.join(process.cwd(), this.availableLogs[logid]['file']), res);
        })
    ...
    readLog(file, res) {
        ...
        fs.stat(file, function(err, data) {
            ...
            if (isNaN(alpha)){ return res.sendStatus(500) }
            if (isNaN(omega)){ return res.sendStatus(500) }

            fs.createReadStream(file, { alpha, omega }).pipe(res);
        });
    }
